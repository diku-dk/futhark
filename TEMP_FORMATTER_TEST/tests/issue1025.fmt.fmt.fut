...
def barycentric_coordinates ({x = x, y = y}: i32racer.point2D) (triangle: triangle_projected) =
  let ({x = xp0, y = yp0, z = _}, {x = xp1, y = yp1, z = _}, {x = xp2, y = yp2, z = _}) = triangle
  in ((yp1 - yp2) * (x - xp2) + (xp2 - xp1) * (y - yp2))

def main [tn] (triangles_projected: [tn]triangle_projected) (w: i32) (h: i32) rects ((n_rects_x, n_rects_y): (i32, i32)) =
  ...
  let rect_in_rect (({x = x0a, y = y0a}, {x = x1a, y = y1a}): rectangle) (({x = x0b, y = y0b}, {x = x1b, y = y1b}): rectangle): bool = not(x1a <= x0b || x0a >= x1b || y1a <= y0b || y0a >= y1b)
  let bounding_box (
      ({x = x0, y = y0, z = _}
      ,{x = x1, y = y1, z = _}
      ,{x = x2, y = y2, z = _}): triangle_projected
    ): rectangle =
  ...
  let each_rect [bn] (rect: rectangle) (pixel_indices: [bn]i32)=
    let rect_triangles_projected = filter (triangle_in_rect rect) triangles_projected
