...
-- compiled input { 0i32 0f64 1f64 0f64 } output { 1f64 0f64 0f64 }
-- compiled input { 1i32 0f64 1f64 0f64 } output { 0.412393f64 0.907265f64 -0.082479f64 }
-- compiled input { 2i32 0f64 1f64 0f64 } output { -0.375775f64 0.903687f64 -0.205287f64 }
...
def rotateY (theta: f64) ({x = x, y = y, z = z}: Vec) =
  let cos_theta = f64.cos theta
  let sin_theta = f64.sin theta
  in
    {x = f64(cos_theta * x - sin_theta * z)
    ,y
...
type Object = #Wall DirectionDistance
|#Block PositionBlockHalfWidthsAngle
|#Sphere PositionRadius

def sdObject (obj: Object) (pos: Position): Distance =
  match obj
    case #Wall nor d -> f64(d + dot nor pos)
    case #Block blockPos halfWidths angle -> let pos' = rotateY angle (pos <-> blockPos)
      in norm (vmap (f64.max 0) (vmap f64.abs pos' <-> halfWidths))
    case #Sphere spherePos r -> let pos' = pos <-> spherePos
      in f64(max (norm pos' - r) 0)

def vec x y z: Vec = {x, y, z}

def unvec ({x = x, y = y, z = z}: Vec) = (x, y, z)
...
entry jvp_normal (obji: i32) x y z =
  let f i=
    jvp
      (sdObject (get_obj obji)) (vec x y z) (
        vec
          (f64.bool (i == 0)) (f64.bool (i == 1)) (f64.bool (i == 2))
      )
  in (f 0, f 1, f 2)
