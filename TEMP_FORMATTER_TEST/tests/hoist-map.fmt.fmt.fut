-- If we don't hoist very aggressively here, this will not run
-- correctly with the GPU backends.
-- ==
-- compiled random input { [100][64]i32 } auto output
local def log2 (n: i64): i64 =
  let r = 0
  let (r, _) =
    loop (r, n) = (r, n) while 1 < n do
      let n = n / 2
      let r = r + 1
      in (r, n)
  in r
  ...
  let d = log2 n
    in
      ...
      let largest = reduce (\x y -> if x <= y then y else x) xs[0] xs
        in
          (concat xs (replicate (2 ** (d + 1) - n) largest)
          ,d + 1)
...
-- | Sort an array in increasing order.
  ...
  in
    (
      loop xs = xs for i < d do
        loop xs = xs for j < i + 1 do kernel_par (<=) xs i j
    )[:n]

def main = map (merge_sort i32.(<=))