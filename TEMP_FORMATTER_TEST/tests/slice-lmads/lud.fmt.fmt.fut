  ...
  map1
    (
      \mat ->
        let mat = copy mat
        in
          loop (mat: *[b][b]f32) = mat for i < b - 1 do
            let col =
              map
                (
                  \j ->
...
def lud_internal [m] [b] (top_per: [m][b][b]f32) (lft_per: [m][b][b]f32) (mat_slice: [m][m][b][b]f32): *[m][m][b][b]f32 =
  ...
  in
    map
      (
        \(mat_arr: [m][b][b]f32, lft: [b][b]f32): [m][b][b]f32 ->
          map
            (
              \(mat_blk: [b][b]f32, top: [b][b]f32): [b][b]f32 ->
                map
                  (
                    \(mat_row: [b]f32, lft_row: [b]f32): [b]f32 ->
                      map
                        (
                          \(mat_el, top_row) ->
...
entry lud [n] (block_size: i64) (mat: *[n]f32): [n]f32 =
  ...
  let mat =
    loop mat = mat for step < num_blocks - 1 do
      -- 1. compute the current diagonal block
      let diag =
        lud_diagonal
          (
            flat_index_2d
              mat (row_length * block_size * step + block_size * step) block_size row_length block_size 1
          )
      let mat =
        flat_update_2d
          mat (row_length * block_size * step + block_size * step) row_length 1 diag
      -- 2. compute the top perimeter
      let top_per_irreg =
        lud_perimeter_upper
          diag (
            flat_index_3d
              mat (row_length * block_size * step + block_size * (step + 1)) (num_blocks - step - 1) block_size block_size row_length block_size 1
          )
      let mat =
        flat_update_3d
          mat (row_length * block_size * step + block_size * (step + 1)) block_size row_length 1 top_per_irreg
      -- 3. compute the left perimeter
      let lft_per_irreg =
        lud_perimeter_lower
          diag (
            flat_index_3d
              mat (row_length * block_size * (step + 1) + block_size * step) (num_blocks - step - 1) (row_length * block_size) block_size row_length block_size 1
          )
      let mat =
        flat_update_3d
          mat (row_length * block_size * (step + 1) + block_size * step) (row_length * block_size) row_length 1 lft_per_irreg
      -- 4. compute the internal blocks
      let internal =
        lud_internal
          top_per_irreg lft_per_irreg (
            flat_index_4d
              mat (row_length * block_size * (step + 1) + block_size * (step + 1)) (num_blocks - step - 1) (row_length * block_size) (num_blocks - step - 1) block_size block_size row_length block_size 1
          )
      let mat =
        flat_update_4d
          mat (row_length * block_size * (step + 1) + block_size * (step + 1)) (row_length * block_size) block_size row_length 1 internal
      in mat
