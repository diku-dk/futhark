...
def findLeaf [q] [d] (median_dims: [q]i32) (median_vals: [q]f32) (height: i32) (query: [d]f32) =
  let leaf =
    loop (node_index) = (0) while not(isLeaf height node_index) do
      if query[median_dims[node_index]] <= median_vals[node_index] then
      (node_index + 1) * 2 - 1
      else
      (node_index + 1) * 2
  in leaf - i32.i64 q

def traverseOnce [q] [d] (radius: f32) (height: i32) (kd_tree: [q](i32, f32, i32)) (query: [d]f32) (last_leaf: i32, stack: i32, dist: f32): (i32, i32, f32) =
  ...
  let getAncSameDimContrib (q_m_i: f32) (node_stack: i32) (node: i32): f32 =
    (
      loop (idx, res) = (node, 0.0f32) while (idx >= 0) do
        ...
        in
          ...
          let anc_lev = getLevel anc
            ...
            in
              if notis_anc_visited then
              (anc, res)
              else
              (-1i32, median_vals[anc] - q_m_i)
    ).1
  let (parent_rec, stack, count, dist, rec_node) =
    loop (node_index, stack, count, dist, rec_node) = (last_leaf, stack, height, dist, -1) for _i2 < height + 1 do
      if (node_index != 0) && (rec_node < 0) then
      let parent = (node_index - 1) / 2
        ...
        in
          if scnd_visited then
          -- continue backing-up towards the root
            (parent, stack, count - 1, dist_minu, -1)
          else
          -- the node_index is actually the `first` child of parent,
            let to_visit = dist_plus <= radius
            in
              if notto_visit then
              (parent, stack, count - 1, dist, -1)
              else
              -- update the stack
      ...
      (node_index, stack, count, dist, rec_node)
  let (new_leaf, new_stack, _) =
    if parent_rec == 0 && rec_node == -1 then
    -- we are done, we are at the root node
      (i32.i64 no_leaf, stack, 0)
    else
    -- now traverse downwards by computing `first`
      loop (node_index, stack, count) = (rec_node, stack, count) for _i3 < height + 1 do
        if isLeaf height node_index then
...
def bruteForce [m] [d] (radius: f32) (query: [d]f32) (query_w: f32) (leaf_refs: [m][d]f32) (leaf_ws: [m]f32): f32 =
  map2
    (
      \ref i ->
...
def iterationSorted [q] [n] [d] [num_leaves] [ppl] (radius: f32) (h: i32) (kd_tree: [q](i32, f32, i32)) (leaves: [num_leaves][ppl][d]f32) (ws: [num_leaves][ppl]f32) (queries: [n][d]f32) (query_ws: [n]f32) (qleaves: [n]i32) (stacks: [n]i32) (dists: [n]f32) (query_inds: [n]i32) (res: f32): ([n]i32, [n]i32, [n]f32, [n]i32, f32) =
  let queries_sorted = gather2D queries query_inds
  let query_ws_sorted = gather1D query_ws query_inds
  let new_res =
    map3
      (
        \query query_w leaf_ind ->
          if leaf_ind >= i32.i64 num_leaves then
          0.0f32
          else
          bruteForce radius query query_w (leaves[leaf_ind]) (ws[leaf_ind])
  ...
  let (new_leaves, new_stacks, new_dists) =
    ...
    <| map4
      (
        \query leaf_ind stack dist ->
          if leaf_ind >= i32.i64 num_leaves then
          (leaf_ind, stack, dist)
          else
          traverseOnce
              radius h kd_tree query (leaf_ind, stack, dist)
...
def propagate [m1] [m] [q] [d] [n] (radius: f32) (ref_pts: [m][d]f32) (indir: [m]i32) (kd_tree: [q](i32, f32, i32)) (queries: [n][d]f32) (query_ws: [n]f32, ref_ws_orig: [m1]f32): f32 =
  ...
  let (_qleaves', _stacks', _dists', _query_inds', res_ws') =
    loop (qleaves: [n]i32, stacks: [n]i32, dists: [n]f32, query_inds: [n]i32, res_ws: f32) = (qleaves, stacks, dists, query_inds, res_ws) for _i < 8 do
      iterationSorted radius h kd_tree leaves kd_ws_sort queries query_ws qleaves stacks dists query_inds res_ws
  in res_ws'

def rev_prop [m1] [m] [q] [d] [n] (radius: f32) (ref_pts: [m][d]f32) (indir: [m]i32) (kd_tree: [q](i32, f32, i32)) (queries: [n][d]f32) (query_ws: [n]f32, ref_ws_orig: [m1]f32): (f32, ([n]f32, [m1]f32)) =
  let f = propagate radius ref_pts indir kd_tree queries
  in vjp2 f (query_ws, ref_ws_orig) 1.0f32

def main [d] [n] [m] [m'] [q] (sq_radius: f32) (queries: [n][d]f32) (query_ws: [n]f32) (ref_ws: [m]f32) (refs_pts: [m'][d]f32) (indir: [m']i32) (median_dims: [q]i32) (median_vals: [q]f32) (clanc_eqdim: [q]i32) =
  let (res, (query_ws_adj, ref_ws_adj)) = rev_prop sq_radius refs_pts indir (zip3 median_dims median_vals clanc_eqdim) queries (query_ws, ref_ws)
