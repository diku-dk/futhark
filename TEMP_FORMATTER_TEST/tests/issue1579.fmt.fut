...
module loess_m = {
  ...
  def filterPadWithKeys [n] 't (p: (t -> bool)) (dummy: t) (arr: [n]t): ([n]t, [n]i64, i64) =
  ...
  --------------------------------------------------------------------------------
  def loess_outer [n] [n_m] (xx: [n]i64) (yy: [n]t) (q: i64) (ww: [n]t) (l_idx: [n_m]i64) (lambda: [n_m]t) (n_nn: i64) =
  ...
  def loess_outer_l [m] [n] [n_m] (xx_l: [m][n]i64) (yy_l: [m][n]t) (q: i64) (ww_l: [m][n]t) (l_idx_l: [m][n_m]i64) (lambda_l: [m][n_m]t) (n_nn_l: [m]i64) =
    #[incremental_flattening(no_intra)]
    map5
      (
        \xx yy ww l_idx (lambda, n_nn) ->
          loess_outer
            xx yy q ww l_idx lambda n_nn
      ) xx_l yy_l ww_l l_idx_l (zip lambda_l n_nn_l)
  
  def l_indexes [N] (nn_idx: [N]i64) (m_fun: i64 -> i64) (n_m: i64) (q: i64) (n_nn: i64): [n_m]i64 =
    -- [n_m]
    tabulate
      n_m (
        \i ->
          ...
          let (init_idx, _) =
            ...
            loop (low, high) = (0i64, N - 1) while low <= high do
              ...
              in if mid_idx >= x then (low, mid - 1) else (mid + 1, high)
          let (idx, _, _) =
            ...
            loop (l_idx, r_idx, span) = (init_idx, init_idx, 1) while span < q do
              ...
              in
                if l_cand == l_idx then
                (l_idx, r_idx, q)
                else
                -- leftmost found, return
                  if l_dist < r_dist || r_cand == r_idx then
                  (l_cand, r_idx, span + 1)
                  else
                  -- expand to the left
                    (l_idx, r_cand, span + 1)
  ...
  def find_lambda [n_m] (y_idx: []i64) (l_idx: [n_m]i64) (m_fun: i64 -> i64) (q: i64) (n_nn: i64): [n_m]t =
    map2
      (
        \l i ->
          let mv = m_fun i
          ...
          let md_i =
            i64.max
              (i64.abs (y_idx[l] - mv)) (i64.abs (y_idx[r] - mv))
            |> T.i64
          in md_i + T.max (((T.i64 q) - (T.i64 n_nn)) / 2) 0
      ) l_idx (iota n_m)
  
  def loess_params [N] (q: i64) (m_fun: i64 -> i64) (n_m: i64) (y_idx: [N]i64) (n_nn: i64): ([n_m]i64, [n_m]t) =
...
entry main [m] [n] (Y: [m][n]f64) (q: i64) (jump: i64) =
  ...
  let (nn_y_l, nn_idx_l, n_nn_l) = map (loess_m.filterPadWithKeys (\i -> not(f64.isnan i)) 0) Y |> unzip3
  ...
  let (l_idx_l, lambda_l) =
    ...
    |> unzip
  let weights_l = replicate (m * n) 1.0f64 |> unflatten
  in
    loess_m.loess_outer_l
      nn_idx_l nn_y_l q weights_l l_idx_l lambda_l n_nn_l