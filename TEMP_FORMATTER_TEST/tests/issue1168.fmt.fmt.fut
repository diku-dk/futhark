...
type^ network'p 'i 'o = {parameter: p
...
def gradientDescentStep [n] 'p 'i 'o (lr: f32) (errF: o -> o -> o) (net: network p i o) (samples: [n](i, o)): network p i o =
  let g = map (gradient errF net) samples |> net.sum n
  in
    {parameter = net.add net.parameter (net.scale (-lr / f32.i64 n) g)
...
def gradientDescent [n] 'p 'i 'o (lr: f32) (mf: f32) (batchSize: i64) (steps: i32) (errF: o -> o -> o) (net: network p i o) (samples: [n](i, o)) (momentum: p) (gen: PRNG): (network p i o, p, PRNG) =
...
def genMap [n] 'p 'i 'o (ps: [n]p) (f: p -> i -> o) (df: p -> i -> o -> (i, p)) (scale': f32 -> p -> p) (add': p -> p -> p) (zero': p): network ([n]p) ([n]i) ([n]o) =
  let parameter = ps
...
def main [m] [n] lr mf batchSize steps stuff (netParameter: networkParameter [m] [n]) (momentum: networkParameter [m] [n]) (samples: []sample) (gen: PRNG) =
  let inet = fromParameter stuff netParameter
  let samples' = map (\((p, o), r) -> ((p, m33fromQuaternion o), r)) samples
  let (_, momentum', _) =
    gradientDescent
      lr mf batchSize steps (\a b -> b - a) inet.net samples' momentum gen
  in momentum'