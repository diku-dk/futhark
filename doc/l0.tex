...
If the type of an array is preceded by a single asterisk, this denotes
...
(parts of) the old value of the array.  In the following, let
$\aliases{e}$ denote the variables that may alias $e$, where $e$ is an
arbitrary expression (although most often a variable).  Whenever we
consume a variable $a$, we mark as inaccessible all of its aliases.
...
The details of how sharing is determined is described in further
detail in section \ref{subsec:l0-sharing}.  For now, assume that the
alias-sets are given, and let us take a closer look at the
interactions between observing and consuming variables.
