...
%
In functional languages, fusion is naturally and relatively easily derived
...
In imperative languages, fusing producer-consumer loops 
requires dependency analysis on arrays applied at loop-nest level.
...
and present a structural-analysis technique, inspired
...
statistics related to fusion on a set of six benchmarks. 
...
computation since it may replace some of the 
array accesses with scalar accesses, 
e.g., if the element type is a basic type.
...
body exhibits non-affine indexing or non-trivial control flow, e.g., loop nests.
...
rules or skeletons~\cite{jones2001playing,Chak06DPH,bergstrom2012nested}.
...
It is essentially syntactic sugar for a certain form of 
...
e.g., loop interchange, distribution.  
%
For example, denoting by {\tt t} the type of {\tt x}, the loop in
Figure~\ref{fig:loop-recursion} has the semantics of 
a call to the tail-recursive function on the right side.
...
reproduce it here for completeness.  Note that the types given are not \LO{}
...
In particular, note that {\tt scan} and {\tt reduce} require binary 
associative operators and {\tt scan} is an inclusive prefix scan.  
{\tt redomap} is a special case -- it is not part of the external \LO{}
...
After an array has been used on the right-hand side of a let-with, 
we mark it and all of its aliases as consumed, and it may not be used
afterwards.
%
Our aliasing analysis is rather conservative.  In particular, we
assume that if a function returns a non-unique array, then that array may
alias any of the function arguments.  We also do not detect aliasing
at a finer granularity than whole arrays, i.e., after {\tt let a = b[0]},
{\tt a} aliases all of {\tt b}, not only its first row.
...
The \LO{} compiler is implemented in Haskell.  Figure~\ref{fig:fusionDS}
shows the structure of the data-flow result of the %computed during the
...
map that binds an array name to the {\sc soac} statement producing it, {\tt soacsEnv}. 
The environment is computed during the top-down \textsc{AbSyn} traversal, 
i.e., an inherited attribute.
%
The computation takes place in the \emp{\tt FusionM} monad that makes
the {\tt FusionEnv} environment available via a {\tt Reader} monad
interface.
...
%
{\tt unionRes} performs the (semantic) union of the results of the two 
branches, e.g., the union of the {\tt unfusable} sets and the union of 
the fusion kernels, etc, because a {\sc soac} can be fused in two kernels 
on separate branches without duplicating computation.
%
...
fusion1 r (\textit{let} id1 \textit{=} id0 \textit{with}
    ...
    let kers = M.map (addToInplace id0) (\emphh{kernels} r')
    let r'' = r' \{ \emphh{kernels} = kers \}
...
fusion1 r (\textit{let} pat \textit{=} rhs \textit{in} body) =
  case rhs of
    \textit{map2}(fun, inputs) -> do
      ...
      r'' <- fusion1 r' lam
      \emp{tryFuseSOAC} (freeInBody fun) r'' (pat, rhs)
...
%            Our actual implementation uses an
...
%            Figure~\ref{fig:dontFuse}.A loop is treated similarly.
...
{\tt allOutArrs} (shown in Figure~\ref{fig:helpers}) 
...
while preserving the parallel semantics of the result.  % fusion's 
For example, a {\tt reduce2} can be fused with a {\tt filter2}, then with 
several (partial) {\tt map2}s, then again with a {\tt filter2}, etc. %and so on. 
...
\section{Discussion, Possible Extensions, and Statistics}
\label{sec:Discuss}
...
\begin{colorcode}
let x=map2(fn int ([int] x) =>   \emphh{\mymath{\equiv}}   reduce2( op +, 0,
         reduce2(op +,0,x), a)           flatten(2, a) )
in  reduce2(op +, 0, x)  
\end{colorcode}
...
%Similarly to \textsc{Iswim}, \textsc{RedFlat} both enables and is enabled by fusion. 
We plan to extend the fusion analysis to incorporate 
...
a sequential loop ({\tt scan}). The fact that the inner loop is parallel
...
which comprises about $1000$ lines of Haskell code. 
%
...
local barriers, rather than multiple execution of a 
{\sc gpu} kernel. 
...
algebra that is a subset of ours was presented in~\cite{bird1989algebraic}.  In
...
pointers'' found in the impure language Rust~\cite{rust}, albeit
...
arrays passed as unique-type parameters, whereas in Rust this
