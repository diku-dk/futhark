...
and regular arrays are more amenable to compiler optimizations, in
particular by simplfying static size analysis.
%
...
%(although SaC loops also have significant performance repercussions).
...
newtype \emp{FusionM} a = FusionM (StateT NameSrc 
               (ReaderT \emp{FusionEnv} (Either FusionErr)) a)
  deriving ( MonadState  NameSrc, MonadReader \emp{FusionEnv},
...
fusion1 r (Var id\mymath{\myindx{0}}) = do 
  if not \$ isArrayType id\mymath{\myindx{0}} then return r
  else do ufus' = S.insert id\mymath{\myindx{0}} (\emphh{unfusable} r)
          return \$ r \{ \emphh{unfusable} = ufus' \}

fusion1 r (LetWith id1 id0 inds elem body \emp{alias} _) = do 
    r'  <- \emphh{bindVarsEnv} [id1] \$ fusion1 r body
    -- Add the aliases set of id0 (included)
    -- to the `inplace' field of any kernel:
    let kers = M.map (\mymath{\backslash}k->k\{(\emphh{inplace} k) `S.union` \emp{alias}\}) 
                     (kernels r')
    let r'' = r' \{ kernels = kers \}
    foldM fusion1 r'' (elem : (Var id0) : inds)

fusion1 r (If cond\mymath{\myindx{e}} then\mymath{\myindx{e}} else\mymath{\myindx{e}} _ _) = do
    then\mymath{\myindx{r}} <- fusion1 mkNullRes then\mymath{\myindx{e}}
    else\mymath{\myindx{r}} <- fusion1 mkNullRes else\mymath{\myindx{e}}
    cond\mymath{\myindx{r}} <- fusion1 r         cond\mymath{\myindx{e}}
    return \$ \emp{composeRes} cond\mymath{\myindx{r}} (\emp{unionRes} then\mymath{\myindx{r}} else\mymath{\myindx{r}})

fusionLam1 r (AnonymFun ids body pos) = do
...
-----------------
-- Let Pattern --
fusion1 r (LetPat pat soac@(Map2 \mymath{\lambda} _ _ _) bdy _)=do
  r' <- \emphh{bindBothEnvs} pat soac \$ fusion1 r bdy    
...
\emp{tryFuseSOAC} lam_vars res (out_nms, soac) = do   
  ...
  if not all4_ok 
  then ... -- add a fresh kernel to the result, and    
           -- update the outArr and inpArr fields
  else ... -- fuse current soac with all to_fuse_kers
