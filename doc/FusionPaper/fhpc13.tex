...
SOACs and operators are variables, and that every tuple-pattern is
fully expanded to cover all elements of the tuple value to be matched.
One notable property of the resulting program is that no variable is
ever bound to a tuple.

The enabling optimisations loop consists of
\begin{enumerate}
\item inlining as much as possible,
\item performing copy- and constant-propagation, and
\item removing dead code,
\end{enumerate}
which is repeated until a fixed point is reached.  After loop fusion,
the enabling optimisations stage is repeated, as fusion will often
produce code amenable to copy propagation.

\subsubsection{Tuple transformation}
\label{sec:tuple-transformation}

The tuple-transformation stage flattens all tuples (i.e, {\tt
  (x,(y,z))} becomes {\tt (x,y,z)}), and converts arrays of tuples to
tuples of arrays.  Arrays of tuples are in a sense merely syntactic
sugar for tuples of arrays; the type {\tt [(int, real)]} is
...
The remaining SOACs are similar.  $f'$ refers to $f$ rewritten to to
...
tuple-SOAC must all be of the same length.
...
The fusion transformation, presented in this section, assumes a
{\em normalized} program, i.e., by running the enabling
transformations introduced in Section~\ref{sec:Prelim}.   
...
\begin{itemize}
    ...
    \item each {\tt if} is binded to a corresponding {\tt let} expression, and an 
...
computation, despite the fact that several of the to-be-fused 
arrays are used in different {\sc soac}s, e.g., {\tt y1} is used to
compute both {\tt (z1,z2)} and {\tt res}. Similarly, we note
that not all input arrays of a {\sc soac} are produced by one other
{\sc soac}, e.g., the {\tt q}s requires both {\tt y}s and {\tt z}s arrays.   
We achieve this by means of {\em {\tt T$_2$} reduction on the dependency graph}:
...
to compute the per-element output of the child, %% intermediate, 
and, finally, (iii) by removing the duplicate input arrays of the result.
The latter is accomplished by replacing in the result's lambda
...
\begin{itemize}
    ...
    \item [5.] If a {\sc soac}-produced array {\tt x} is used other than input to   
                another {\sc soac} then fusing {\tt x} is still disallowed in our 
                implementation.   A future extension might handle the cases exhibiting
                ``negligible'' overhead, e.g., substituting {\tt x[i]} with {\tt f(arr[i])} 
                would allow fusion between the producers of {\tt x} and {\tt y} at the cost 
                of an extra call to {\tt f}.
\end{itemize}
