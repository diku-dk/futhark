...
Since these transformations both enables and are enabled by fusion,
...
let (.., re\mymath{\myindx{t}}, ..) = (.., map\mymath{\myindu{n}}( replicate(s), ne\mymath{\myindx{t}} ), ..)
...
\begin{description}
%\begin{itemize}
\item[P0:] a real-world pricing kernel for financial derivatives~\cite{PricingFHPC}, %PricingLexiFi
\item[P1:] a real-world market calibration kernel that computes some of the parameters of \textbf{P0},%CalibLexiFi
\item[P2:] stochastic volatility calibration via Crank-Nicolson finite differences solver~\cite{} %HiperfitEgCos
\item[P3:] %BabyBear
...
Lines Of Code          & 283 & 859 & 182 & 29 & 21 & 22\\\hline\hline
...
rewrite rules to perform fusion, including expressing array operations
in terms of streams~\cite{coutts2007rewriting}, which have previously
been shown to be easily fusable.  While DPH obtains good results,
rewrite rules are quite limited -- they are an inherently local view
of the computation, and would be unable to cope with limitations in
the presence of in-place array updates, and whether the result of an
array operation is used multiple times.  The Glasgow Haskell Compiler
itself also bases its list fusion on rewrite
rules~\cite{jones2001playing}.
...
although it's unclear how it's handled if the output of an array
operation is used as input to multiple other array operations.
