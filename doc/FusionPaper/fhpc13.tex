...
%We have extended our fusion analysis to overcome such cases by 
%associating calls to {\tt size} (and {\tt assertZip}) with the 
%kernels who use the argument of {\tt size} as input, and by
%performing the translation {\tt size(x) $\Rightarrow$ size(a)} 
%at the time when {\tt x} is fused.

\begin{itemize}
    \item Discuss hindrances for fusion: {\tt assertZip}, {\tt size} 
...
// \emp{Generalization for Nested Map2s (similar for redomap2)}
...
// \emp{similar for Interchange Reduce with Inner Maps(IRWIM)}
