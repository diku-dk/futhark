...
must not alias any non-unique arguments.
...
%  \item[{\tt mapall2($l$, $e_{1}$, \ldots, $e_{n}$)} $\rightarrow$]
%    {\tt unzip(}
...
Since $\mathcal{L}_0$ guarantees a reducible {\sc cfg}, %at function level,
fusion is implemented as an intra-procedural\footnote{
The current version of the $\mathcal{L}_0$ compiler 
...
computation despite the fact that several of the to-be-fused variables
are used in different {\sc soac}s, e.g., {\tt y1} is used to
compute both {\tt (z1,z2)} and {\tt res}. We achieve this means of
{\em {\tt T$_2$} reduction on the dependency graph}:
The rightmost child of the root {\sc soac} has only one incoming edge, 
hence it can be fused (reduced). This is achieved by (i) replacing in the
root {\sc soac} the child's output arrays of the child's input array,
and inserting a call  
