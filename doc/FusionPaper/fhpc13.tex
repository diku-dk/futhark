...
data \emp{FusedKer} = \emp{FusedKer} \{ 
 \emphh{soacStmt} :: ([Ident], Exp)
--^the fused SOAC stmt,eg,
--(z,w)=map2( f(a,b), x,y )

,\emphh{inp}     :: S.Set Ident
...
fusionGather r (Var idd) = do
  ...
  case identType idd of
    Array\{\} -> return \$ r \{ unfusable = 
                   S.insert (identName idd) (unfusable r) \}
    _       -> return \$ r
...
\begin{itemize}
    ...
    \item  an indexed-array expression adds the array name to the 
            unfusable set and recursively process the expressions corresponding
            to the array indices,
    \item  an if-then-else 
\end{itemize}
