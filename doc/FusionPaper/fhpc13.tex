...
Figure~\ref{fig:T1T2} depicts the intuitive idea that lays the
foundation of the fusion analysis.   The top-left figure shows
the {\em dependency graph} of a simple program, where the arrows
point from the consumer to the producer. The program is a basic
block, i.e., no branches or loops, in which (array) variables 
are used only as input or output to {\sc soac} calls.   
...
computation despite the fact that several of the fused variables
are used in different {\sc soac}s. For example  
Despite the fact that several variables are used in different {\sc soac}s
all

belonging to kernel $3$, One can observe that several array variables
are used in different {\sc soac}s, e.g.,  
