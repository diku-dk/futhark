...
\begin{itemize}
    \item Delayed Arrays: index-function repres., e.g., Repa, Obsidian\smallskip
            \begin{itemize}
                ...
                \item may generate ugly indexing, e.g., non-coalesced memory access\bigskip
            \end{itemize}
...
\begin{frame}[fragile,t]
  ...
  The {\tt {\emp *}} denotes a {\em unique} value.  Unique arguments are not
...
\pause
\begin{block}{List homomorphism (using Haskell notation)} {\tt (red $\odot$ e) . (map f)} can
  be transformed to:  \\
  ...
  \begin{block}{How About This one}
    \begin{colorcode}[fontsize=\scriptsize]
      ...
      fun real main([real] x1, [[real]] x2) =
        let (\emp{y1}, \emp{y2}, \emp{y3}) = map( f1, x1, x2[1] )        in
        let (\empr{z1}, \empr{z2})     = map( f2, \emp{y1}, \emp{y2} )           in
        let (\emph{q1}, \emph{q2})     = map( g , \emp{y3},\empr{z1},\emp{y2},\emp{y3} )      in
        let res          = map( myop, \emph{q1},\emph{q2},\empr{z2},\emp{y1},\emp{y3} ) in
        res[3]
    \end{colorcode}
  \end{block}
...
let x1 = map(f0, x2) in
let x3 = map(f, x1) in
let (\emp{y1}, \emp{y2}, \emp{y3}) =
  map(f1, x1, x2[1]) in
let (\emphb{z1}, \emphb{z2}) =
  map(f2, \emp{y1}, \emp{y2}) in
let (\emphh{q1}, \emphh{q2}) =
  map(g, \emp{y3}, \emphb{z1}, \emp{y2}, \emp{y3}) in
let res =
  map(h, \emphh{q1}, \emphh{q2}, \emphb{z2}, \emp{y1}, \emp{y3}) ...
\end{colorcode}
\end{minipage}
&
$\Rightarrow$
...
\begin{frame}[fragile,t]
  ...
  In the future, write a code generator; implement transformations
  that ease weaknesses in the fusion algebra (e.g. ISWIM).
