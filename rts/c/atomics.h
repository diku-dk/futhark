...
#ifdef FUTHARK_CUDA
  return atomicExch_system((int32_t*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicCAS_system((int32_t*)p, cmp, val);
#else
...
#ifdef FUTHARK_CUDA
  return atomicAdd_system((int32_t*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicAdd_system((float*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicMax_system((int32_t*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicMin_system((int32_t*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicMax_system((uint32_t*)p, x);
#else
  return atomic_max(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicMin_system((uint32_t*)p, x);
#else
  return atomic_min(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicAnd_system((int32_t*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicOr_system((int32_t*)p, x);
#else
  return atomic_or(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicXor_system((int32_t*)p, x);
#else
  return atomic_xor(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicExch_system((uint64_t*)p, x);
#else
  return atom_xor(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicCAS_system((uint64_t*)p, cmp, val);
#else
...
#ifdef FUTHARK_CUDA
  return atomicAdd_system((uint64_t*)p, x);
#else
  return atom_add(p, x);
#endif
}
...
inline double atomic_fadd_f64_global(volatile __global double *p, double x) {
  ...
  return atomicAdd_system((double*)p, x);
#else
...
#ifdef FUTHARK_CUDA
  return atomicMax_system((int64_t*)p, x);
#else
  return atom_max(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicMin_system((int64_t*)p, x);
#else
  return atom_min(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicMax_system((uint64_t*)p, x);
#else
  return atom_max(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicMin_system((uint64_t*)p, x);
#else
  return atom_min(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicAnd_system((int64_t*)p, x);
#else
  return atom_and(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicOr_system((int64_t*)p, x);
#else
  return atom_or(p, x);
#endif
}
...
#ifdef FUTHARK_CUDA
  return atomicXor_system((int64_t*)p, x);
#else
  return atom_xor(p, x);
#endif
}
