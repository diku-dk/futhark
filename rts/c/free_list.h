/* An entry in the free list.  May be invalid, to avoid having to
   ...
   NOTICE: Requires a defintion of the fl_entry_mem type*/
...
void free_list_insert(struct free_list *l, size_t size, fl_entry_mem mem, const char *tag) {
...
int free_list_find(struct free_list *l, const char *tag, size_t *size_out, fl_entry_mem *mem_out) {
  int i;
  for (i = 0; i < l->capacity; i++) {
...
int free_list_first(struct free_list *l, fl_entry_mem *mem_out) {
  for (int i = 0; i < l->capacity; i++) {
    if (l->entries[i].valid) {
      l->entries[i].valid = 0;
      *mem_out = l->entries[i].mem;
