...
fn map_transpose_NAME_low_width(
    ...
    for (var i1 = 0; i1 <= args.repeat_1; i1++) {
        ...
        for (var i2 = 0; i2 < args.repeat_2; i2++) {
            ...
            var x_index = tblock_id_0 * TR_BLOCK_DIM + i32(local_id[0]) / args.muly;
            var y_index = tblock_id_1 * TR_BLOCK_DIM * args.muly + 
                          i32(local_id[1]) + 
                          (i32(local_id[0]) % args.muly) * TR_BLOCK_DIM;
            ...
            x_index = tblock_id_1 * TR_BLOCK_DIM * args.muly +
                      i32(local_id[0]) + 
                      (i32(local_id[1]) % args.muly) * TR_BLOCK_DIM;
            y_index = tblock_id_0 * TR_BLOCK_DIM + i32(local_id[1]) / args.muly;
