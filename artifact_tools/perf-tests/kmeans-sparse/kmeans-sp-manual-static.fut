...
--        while (if fix_iter then true else delta > threshold) && i < num_iterations do
            ...
            let diffs =
              map (\row -> 
                    map (\cluster->
                          -- Safety is checked statically by PropProp
                          -- so we can tell the Futhark compiler to remove
                          -- dynamic boundschecking:
                          #[unsafe]     -- remove me and you'll see 2x slowdown
