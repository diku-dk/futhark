...
def datamap(d):
    m = { '#6 (\"4000i32 30.0f32 50i32 1i32 1.0f32\")':
          '#5 (\"4000i32 30.0f32 50i32 1i32 1.0f32\")',

          '#5 (\"8000i32 8000i32 -0.7f32 0.0f32 3.067f32 100i32 16....\")':
          '#4 ("8000i32 8000i32 -0.7f32 0.0f32 3.067f32 100i32 16....")',

          '#5 (\"10.0f32 8000i32 8000i32\")':
          '#4 ("10.0f32 8000i32 8000i32")'}
    if d in m:
        return m[d]
    else:
        return d
...
def release_for_version(version):
    return os.path.join(RELEASES_DIR, 'futhark-{}-linux-x86_64'.format(version))

def tarball_for_version(version):
    return os.path.join(RELEASES_DIR, 'futhark-{}-linux-x86_64.tar.xz'.format(version))

def futhark_compile_for_version(version):
    if version_before(version, '0.9.1'):
        return os.path.abspath(os.path.join(release_for_version(version), 'bin', 'futhark-{}'.format(FUTHARK_BACKEND)))
    else:
        return os.path.join(release_for_version(version),
                            'bin/futhark {}'.format(FUTHARK_BACKEND))

def futhark_bench_for_version(version):
    bs = ' '.join(list(benchmarks.keys()))
    if version_before(version, '0.9.1'):
        prog=os.path.abspath(os.path.join(release_for_version(version),
                                          'bin',
                                          'futhark-bench'))
        compiler=os.path.abspath(os.path.join(release_for_version(version),
                                              'bin',
                                              'futhark-{}'.format(FUTHARK_BACKEND)))
        return '{} -r {} --compiler={} {}'.format(prog, BENCH_RUNS, compiler, bs)
    else:
        prog=os.path.abspath(os.path.join(release_for_version(version),
                                          'bin',
                                          'futhark'))
        return '{} bench -r {} --backend={} --futhark={} {}'.format(prog, BENCH_RUNS, FUTHARK_BACKEND, prog, bs)

def tarball_url(version):
    return 'https://futhark-lang.org/releases/futhark-{}-linux-x86_64.tar.xz'.format(version)
...
def ensure_release(version):
    if not os.path.exists(release_for_version(version)):
        ...
        shell('cd {} && tar xf ../{}'.format(RELEASES_DIR,
                                                  tarball_for_version(version)))
...
def ensure_benchmarks(version):
    if version in versions_to_benchmark_commits:
        ...
        shell('cd {} && git checkout -f {}'.format(FUTHARK_BENCHMARKS_DIR,
                                                    versions_to_benchmark_commits[version]))
        return GITS_DIR
    else:
        d = ensure_futhark_repo()
        tag = 'master' if version == 'nightly' else 'v{}'.format(version)
        shell('cd {} && git checkout -f {} && git submodule update'.format(d, tag))
...
def produce_runtime_json(version):
    ...
    shell('cd {} && {} --json={}'.format(d,
                                         futhark_bench_for_version(version),
                                         json_f))
...
def produce_compile_json(version):
    ...
    for b in benchmarks:
        bef = time.time()
        shell('{} {}'.format(futhark_compile_for_version(version),
                             os.path.join(d,b)))
        aft=time.time()
...
def plot_runtime(versions):
    ...
    for b,d in benchmarks.items():
        results_per_bench[b,d] = np.array(results_per_bench[b,d]) / fastest[b,d]
    ...
    ax.legend(bbox_to_anchor=(0, -0.35),
              ncol=3,
              loc='center left',
              fontsize=5.5)
...
def plot_compiletime(versions):
    ...
    for b in benchmarks:
        ...
        ax.plot(xs, ys,
                label=b[len('futhark-benchmarks/'):])
