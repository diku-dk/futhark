...
def mpe(runtimes: np.ndarray = None, **kwargs) -> str:
    ...
    mpe = 100 / runtimes.shape[0] * ((runtimes - runtimes.mean()) / runtimes).sum()
    return f"{mpe:.2e}%"
...
class PlotType(ABC):
    ...
    def find_str_formatter(cls, z) -> Tuple[FormatStrFormatter, float]:
        '''
        Tries to find a suitable time unit for the given numpy array. The scaling factor is returned
        and a string formatter for matplotlib axis using the suitable unit is returned.
        '''
        units = [
...
def make_html_descriptors(plot_job: PlotJob) -> str:
    ...
    def row(name, func):
        return rf"<tr><td>{name}:</td><td>{func(**plot_job.benchmark_result)}</td></tr>"
...
def make_html(folder_content: Dict[str, List[str]], plot_jobs: Dict[str, PlotJob], root: str) -> str:
    ...
    folder_keys = {folder : make_key(folder, 32) for folder in folder_content.keys() }
    ...
    def make_list(path: str) -> str:
        '''
        Creates the list which shows the structure of benchmarks and links to the sections.
        '''
        directory = list(map(lambda p: os.path.join(path, p), sorted(os.listdir(path))))
                        
        if len(directory) == 1 and os.path.isdir(directory[0]) and path == root:
    ...
    def make_section(folder: str, dataset_plot_files: List[str]) -> str:
        '''
        Makes a section with all the plots and descriptors for a given benchmark's datasets.
        '''
        subsections_data = map(lambda a: (a, plot_jobs.get(a)), dataset_plot_files)
        subsections = ''.join(map(lambda a: make_subsection(*a), subsections_data))
