        ...
        try:
            ...
            for dataset in benchmark_results['datasets']:
                if type(benchmark_results['datasets'][dataset]) is dict:
                    runtimes[cut_desc(dataset)] = np.mean(
                        benchmark_results['datasets'][dataset]['runtimes'])
...
for r in results:
    ...
    for dataset in r['runtimes']:
        if dataset not in best or \
           r['runtimes'][dataset] < best[dataset]['runtime']:
...
for dataset in sorted(best.keys()):
    ...
    print 'Dataset {} best: {} {} {}'.format(
        dataset, best_time, best[dataset]['commit'], best[dataset]['runtime']
    )
