...
-- auto output
-- structure gpu { /WithAcc/If 1 }

import "../../accs/intrinsics"

def update [n] (acc: *acc ([n]i64)) (flag: bool, i: i64, x: i64) : acc ([n]i64) =
  if flag
  then let z = replicate 10 x with [3] = x
       let res = i64.sum z
       in write acc i res
  else let z = replicate 20 x with [3] = -x
       let res = i64.sum z
       in write acc (i) res
