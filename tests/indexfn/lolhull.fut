...
def expand_hull [num_segs] [num_points]
  ...
  let (extrema_ix0, _) = unzip extrema_ix
  ...
  let new_seg_inds =
    map3 (\ix seg_ix p ->
        let extreme_ix = extrema_ix0[seg_ix]
        in if extreme_ix == ix then -1 else
        let (a, b) = segs[seg_ix]
        let q = points1[extreme_ix]
...
def extract_empty_segments [num_segs] [num_points]
  ...
  let (segs_false0, _) = unzip segs_false
  let hull' = (++) hull segs_false0
  let segs' = segs_true
  let sgm_inds' =
...
def semihull (start : point) (end : point) (points : []point) =
  if null points then [start]
    ...
    (loop (hull, segs, points) =
     ...
     let (segs0, segs1) = unzip segs
     let (points0, points1) = unzip points
     let (segs', points') = expand_hull segs0 segs1 points0 points1 
     let (seg_inds', only_points') = unzip points'
     let (segs'0, segs'1) = unzip segs'
     let (hull', segs'', sgm_inds'') = extract_empty_segments hull segs'0 segs'1 seg_inds'
...
def reduce [n] 't f ne (xs: [n]t): t =
  if n > 0 then (scan f ne xs)[n-1] else ne
...
-- def sort_by [n] 't (f: t -> f64) (xs: [n]t): [n]t =
--   radix_sort_float_by_key f f64.num_bits f64.get_bit xs
def sort_by [n] 't (f: t -> f64) (xs: [n]t): [n]t =
  ???
