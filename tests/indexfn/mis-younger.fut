...
def can_add [V] [E]
            (shape: {[V]i64 | \x -> Range x (0,E+1)})
            (offsets: {[V]i64 | \x -> Range x (0,E+1) && Monotonic (<=) x})
  ...
  if (C[index] == 0) then
    ...
    let num = shape[index]
    let end = if vEntry + num < E then vEntry + num else 0
...
def remove_neighbour_and_self [V] (marked: []i64) (targets: []i64)
                                       (C: *[V]i64): *[V]i64 =
...
def mis_step_ [V] [E]
             (shape: {[V]i64 | \x -> Range x (0,E+1) && Equiv E (sum x)})
             -- (offsets: {[V]i64 | \x -> Range x (0,E+1) && Monotonic (<=) x && Equiv x (scan (+) 0 shape)})
             (offsets: {[V]i64 | \x -> Range x (0,E+1) && Monotonic (<=) x})
             (edges: {[E]i64 | \x -> Range x (0, V)})
             (newI: [V]bool)
             (C: *[V]i64) (I: *[V]i64)
  ...
  let targets = map2 (\added j -> if added then j else -1) newI (iota V)
  ...
  let szs = map (\v -> if newI[v] then shape[v] else 0) (iota V)
...
def mis_step [V] [E]
  ...
  let newI = map (\i -> can_add shape offsets edges random_state C i) (iota V)
  in mis_step_ shape offsets edges newI C I

let MIS [V] (E: i64)
                 (shape: {[V]i64 | \x -> Range x (0,E+1)})
                 (edges: {[]i64 | \x -> Range x (0, V)})
                 (random_state: [V]i64) (C: *[V]i64)
                 (I: *[V]i64)
                 : {[]i64 | \_ -> true} =
...
--
-- def main [V] [E] (vertexes_enc: {[V]i32 | \v -> Range v (0,
-- E) && Monotonic (<=) v}) (edges_enc: {[E]i32 | \e -> Range e (0,
-- V)}): {[]i32 | \_ -> true} =
--
-- def main [V] [E] (vertexes_enc: [V]i32)
--                            (edges_enc: [E]i32) =
--   let indexes = iota V
...
--   let extended_vertexes = vertexes_64 ++ (map (\_ -> E) (iota 1))
--   let shape = map (\i -> extended_vertexes[i+1] - extended_vertexes[i]) indexes

--   let res = MIS E shape edges_64 random_state C I
