...
let particlePairs [n] (pairs:[n](v3, v3)) : network ([n](v3,v3)) (v3, m33) ([n]v3) =
  ...
  let propagation ps (p, R) =
    ...
    let backward os =
      let i =
        let pg = sumV3s os
        let Rg = map2 (\(_, v) o -> v3outer o v) ps os |> sumM33s -- check outer!!!
        in (pg, Rg)
      let g =
...
let main [m][n] lr mf batchSize steps stuff
  ...
  let (_, momentum', _) = gradientDescent lr mf batchSize steps (\a b -> b - a)
                                          inet.net samples' momentum gen
  in momentum'
