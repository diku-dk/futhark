  ...
  def l_indexes [N]
    ...
    tabulate n_m (\i ->
                    ...
                    let (idx, _, _) =
                      ...
                      loop (l_idx, r_idx, span) = (init_idx, init_idx, 1)
                      while span < q do
                        -- O(q)
                        ...
                        in if l_cand == l_idx
                           then (l_idx, r_idx, q)
                           else -- leftmost found, return
                           if l_dist < r_dist || r_cand == r_idx
                           then (l_cand, r_idx, span + 1)
...
entry main [m] [n] (Y: [m][n]f64) (q: i64) (jump: i64) =
  ...
  in loess_m.loess_outer_l nn_idx_l
                           nn_y_l
                           q
                           weights_l
                           l_idx_l
