...
def lud_internal [m] [b] (top_per: [m][b][b]f32) (lft_per: [m][b][b]f32) (mat_slice: [m][m][b][b]f32) : *[m][m][b][b]f32 =
  ...
  in map (\(mat_arr: [m][b][b]f32, lft: [b][b]f32) : [m][b][b]f32 ->
...
entry lud [n] (block_size: i64) (mat: *[n]f32) : [n]f32 =
  ...
  let mat =
    loop mat for step < num_blocks - 1 do
      -- 1. compute the current diagonal block
      let diag =
        lud_diagonal (flat_index_2d mat
                                    (row_length * block_size * step + block_size * step)
                                    block_size
                                    row_length
                                    block_size
                                    1)
      ...
      let top_per_irreg =
        lud_perimeter_upper diag
                            (flat_index_3d mat
                                           (row_length * block_size * step + block_size * (step + 1))
                                           (num_blocks - step - 1)
                                           block_size
                                           block_size
                                           row_length
      ...
      let lft_per_irreg =
        lud_perimeter_lower diag
                            (flat_index_3d mat
                                           (row_length * block_size * (step + 1) + block_size * step)
                                           (num_blocks - step - 1)
      ...
      let internal =
        lud_internal top_per_irreg
                     lft_per_irreg
                     (flat_index_4d mat
                                    (row_length * block_size * (step + 1) + block_size * (step + 1))
                                    (num_blocks - step - 1)
                                    (row_length * block_size)
                                    (num_blocks - step - 1)
                                    block_size
