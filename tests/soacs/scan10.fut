-- Prefix sum exercising stage-2 loop virtualisation in the two-pass
-- scan (segscan_num_tblocks > scan_stage2_tblock_size).  Tuning sets
-- scan_stage2_tblock_size=4 and segscan_num_tblocks=8 so stage-2
-- needs ceil(8/4)=2 iterations.  Small input so this can run under
-- oclgrind.
--
-- Only OpenCL uses the two-pass scan; CUDA and HIP use single-pass.
-- ==
-- tags { no_cuda no_hip }
-- input { [1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32,
--          1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32,
--          1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32,
--          1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32, 1i32] }
-- output { [1i32, 2i32, 3i32, 4i32, 5i32, 6i32, 7i32, 8i32,
--           9i32, 10i32, 11i32, 12i32, 13i32, 14i32, 15i32, 16i32,
--           17i32, 18i32, 19i32, 20i32, 21i32, 22i32, 23i32, 24i32,
--           25i32, 26i32, 27i32, 28i32, 29i32, 30i32, 31i32, 32i32] }

def main (a: []i32) : []i32 = scan (+) 0 a
