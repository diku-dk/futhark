...
let badd [ipb][n] (q:i32) (as : [(ipb*n)][4]u64) (bs : [(ipb*n)][4]u64) : [(ipb*n)*4]u64 =
  ...
  let rsh =
    loop rsh = ash for i < q do
      let tmparg1 =
          if (i % 2 == 0) then ash else bsh
          -- BUG: when removing the opaque <| #[toregmem(1)] it does not validate anymore! 
      let tmparg1'= #[inform_pardim_only(1)] manifest tmparg1
      let rsh'= #[toregmem(1)] map2 pairAddMap rsh tmparg1'
      let tmparg2 =
          if (i % 3 == 0) then bsh else ash
      let tmparg2' = #[inform_pardim_only(1)] manifest tmparg2
      in  #[toregmem(1)] map2 pairAddMap rsh' tmparg2'
