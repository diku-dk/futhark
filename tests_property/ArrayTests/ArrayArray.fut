...
entry gen_matrix_sums_bad (size: i64) (seed: u64) : [][]i32 =
  ...
  in if r < 2
     then xss
     else -- swap first two rows
          let a = xss[0]
          let b = xss[1]
          in [b] ++ [a] ++ drop 2 xss

-- Helper: clamp index to [0..n-1]
def clamp_index (n: i64) (i: i32) : i64 =
  let ii = i64.i32 i
  in if n <= 0
     then 0
     else if ii < 0
...
def remove_at (i: i32) (xs: []i32) : []i32 =
  let n = length xs
  in if n == 0
     then xs
     else let ii = clamp_index n i
          let pre = take ii xs
          let post = drop (ii + 1) xs
          in pre ++ post

-- Remove row i from a 2D array.
def remove_row (i: i32) (xss: [][]i32) : [][]i32 =
  let r = length xss
  in if r == 0
     then xss
     else let ii = clamp_index r i
          let pre = take ii xss
          let post = drop (ii + 1) xss
          in pre ++ post

def remove_col (j: i32) (xss: [][]i32) : [][]i32 =
  let r = length xss
  in if r == 0
     then xss
     else let c = length xss[0]
          in if c <= 1
             then xss
             else let jj = clamp_index c j
                  let newc = c - 1
...
def update_cell_rc [r] [c] (ri: i64) (ci: i64) (xss: [r][c]i32) : [r][c]i32 =
  let r_ok = 0 <= ri && ri < r
  let c_ok = 0 <= ci && ci < c
  in if !(r_ok && c_ok)
     then xss
     else let old = xss[ri][ci]
...
entry shrink_matrix (xss: [][]i32) (random: u64) : [][]i32 =
  ...
  in if t < rowCount
     ...
     else if t < rowCount + colCount
     then -- remove column j = t-rowCount
          let j: i32 = i32.i64 (t - rowCount)
          in remove_col j xss
     else if t < rowCount + colCount + scalarCount
     then -- shrink one cell k = t-rowCount-colCount
          ...
          in if r == 0 || c == 0
             then xss
                  ...
                  in if new == old
                     then -- no-op: advance tactic
                          xss
                     else let xss_rc': [r][c]i32 =
                            tabulate r (\i ->
                                          tabulate c (\j ->
                                                        if i == ri && j == ci then new else xss_rc[i][j]))
                          let xss': [][]i32 = xss_rc' :> [][]i32
                          in xss'
     else -- exhausted tactics
          xss
