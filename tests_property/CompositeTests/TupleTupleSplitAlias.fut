...
def inner_eq (r1: tupleLeft) (r2: tupleRight) : bool =
  r1.0 == r2.0 && r1.1 == r2.1
...
entry gen_simple_fail (size: i64) (seed: u64) : tuple =
  let rng0 = rng_engine.rng_from_seed [i32.u64 seed]
  let (_, v) = rand_i32.rand (-100i32, 100i32) rng0
  in ((v, v + 1), (v + 1, v + 1))
...
entry shrink_simple (r: tuple) (random: u64) : tuple =
  ...
  in if tactic == 0
     ...
     then let y0' = step0 r.1.0
          let r' = (r.0, (y0', r.1.1))
          in r'
     else let y1' = step0 r.1.1
          let r' = (r.0, (r.1.0, y1'))
          in r'
