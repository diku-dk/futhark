    ...
    testCase "slice . rotate . permute . slice . iota 3" $
      compareOps $
        ...
        let ixfun = permute (slice (iota [n, n, n]) slice3) [1, 0]
            ...
            negslice = DimIndices [DimSlice 0 n1 1, DimSlice (m1 - 1) m1 (-1)]
    ...
    testCase "permute . rotate . slice . permute . slice . iota" $
      compareOps $
        ...
        let slice33 =
            ...
            m = n `P.div` 3
            slice1 = DimIndices [DimSlice (n - 1) n (-1), DimSlice 2 (m - 2) 1]
    ...
    testCase "reshape . permute . rotate . slice . permute . slice . iota 1" $
      compareOps $
        let newdims =
            ...
            m = n `P.div` 3
            slice1 = DimIndices [DimSlice 0 n 1, DimSlice (n -1) n (-1), DimSlice (n -1) n (-1), DimSlice 1 (m -2) (-1)]
