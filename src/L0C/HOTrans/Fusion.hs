...
fuseProg prog = do
    ...
    ks0 <- runFusionGatherM prog (mapM fusionGatherFun prog) env
    ...
    else do prog' <- runFusionGatherM prog (mapM fuseInFun (zip ks prog)) env
            return (True, prog')
...
updateLamSOAC :: Lambda Type -> Exp Type -> FusionGM (Exp Type)
