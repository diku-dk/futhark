...
greedyFuse is_repl lam_used_nms res (idd, soac) = do
    ...
    let ok_sz=case soac of
                Filter2{} ->
                  let ok x = fromMaybe True $ do
                               ...
                               return $ not $ any ((== "shape") . getBuiltinFunName) is
                  in all ok out_nms
                _ -> True
    ...
    where
        ...
        getBestInpArr [] = badFusionGM $ EnablingOptError (srclocOf soac)
                           "In Fusion.hs, getBestInpArr argument empty!"
        getBestInpArr (arr:arrs) =
            foldM (\b x -> case x of
                               Iota{} -> return x
                               Var {} -> case b of
                                            Iota{} -> return b
                                            _      -> return x
                               Index{}-> return b
                               _     -> badFusionGM $ EnablingOptError (srclocOf b)
...
fusionGatherExp fres (LetPat pat soac@(Replicate n el pos) body _) = do
    ...
    let repl_id = Ident repl_idnm (Elem Int) pos
        (lame, rwt) = case typeOf el of
                        ...
                        t                -> (TupLit [el] pos, [t])
        repl_lam = AnonymFun [toParam repl_id] lame (toDecl $ typeOf lame) pos
    ...
    case M.lookup nm (zips fres) of
        ...
        Just (i,ss)-> do
          ...
          case S.toList ss of
            _ | fnm /= "shape" ->  bad
            [Iota n_exp _] ->
              return $ LetPat pat n_exp body' pos -- shape eliminated
            [arr]          ->
              return $ LetPat pat (Shape arr p1) body' pos
            _ -> bad
...
reduce2Redomap (Reduce2 lam ne arrs rwtps pos) = do
    ...
    let map_lam = AnonymFun (map toParam lam_ids) lam_body (toDecl $ typeOf lam_body) pos
