...
atomicHistogram pat space histops kbody = do
  ...
  body <- collect $ do
    ...
    generateChunkLoop "SegHist" False mempty mempty [] $ \flat_idx -> do
...
subHistogram pat space histops num_histos kbody = do
  ...
  body <- collect $ do
    ...
    generateChunkLoop "SegRed" False mempty mempty [] $ \i -> do
...
compileSegHistBody pat space histops kbody = collect $ do
  ...
  generateChunkLoop "SegHist" False mempty mempty [] $ \idx -> do
