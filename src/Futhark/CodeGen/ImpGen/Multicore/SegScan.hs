    ...
    fbody <- collect $ do
      ...
      sWhile (tvExp block_idx .<. tvExp block_no) $ do
        ...
        prefix_seqs <- forM (zip [0..] pts) $ \(k,pt) ->
          dPrimSV ("prefix_seq_" <> nameFromText (prettyText (show k))) pt
        ...
        sIf
          ...
          ( do
              ...
              forM_ (zip scan_ops per_op_prefix) $ \(op, pfxs) ->
                forM_ (zip (xParams op) pfxs) $ \(p, pfx) ->
                  copyDWIMFix (paramName p) [] (Var $ tvVar pfx) []
              ...
              sWhile (tvExp j .<. tvExp chunk_length) $ do
                ...
                compileStms mempty (bodyStms kbody) $ do
                  ...
                  forM_ (zip scan_ops per_scan_res) $ \(op, scan_res_op) ->
                    forM_ (zip (yParams op) scan_res_op) $ \(p, kr) ->
                      copyDWIMFix (paramName p) [] (kernelResultSubExp kr) []
                  
                forM_ (zip scan_ops per_scan_pes ) $ \(op, pes_op) -> do
                  compileStms mempty (bodyStms $ lamBody op) $ do
                    forM_ (zip3 (map resSubExp $ bodyResult $ lamBody op)
                                (xParams op)
                                pes_op) $ \(se, px, pe) -> do
              ...
              forM_ (zip scan_ops per_op_prefs) $ \(op, prefAs) -> do
          ...
          ( do
              ...
              compileStms mempty (bodyStms kbody) $ do
                forM_ (zip scan_ops per_scan_res) $ \(op, scan_res_op) -> do
              ...
              sWhile (tvExp j .<. tvExp chunk_length) $ do
                ...
                j <-- tvExp j + 1
              -- write to aggr array
              forM_ (zip scan_ops per_op_aggr) $ \(scan_op, aggrArr) ->
                      ...
                      sIf 
                        ...
                        forM_ (zip scan_ops per_op_prefixes_var) $ \(scan_op, prefix_vars) ->
                          compileStms mempty (bodyStms $ lamBody scan_op) $ do
                            forM_ (zip (map resSubExp $ bodyResult $ lamBody scan_op)
                                        prefix_vars) $ \(op_res, prefix_var) -> do
