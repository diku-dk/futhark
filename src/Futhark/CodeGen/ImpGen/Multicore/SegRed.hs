...
compileSegRed' pat space reds nsubtasks kbody
  ...
  --     [_] -> nonsegmentedReduction pat space reds nsubtasks kbody
  --     otherwise -> segmentedReduction pat space reds kbody
...
segmentedReduction :: Pattern MCMem
                   ...
                   -> DoSegBody
                   -> MulticoreGen Imp.Code
segmentedReduction pat space reds kbody = collect $ do
...
compileSegRedBody :: VName
                  ...
                  -> DoSegBody
                  -> MulticoreGen Imp.Code
compileSegRedBody n_segments pat space reds kbody = collect $ do
  let (is, ns) = unzip $ unSegSpace space
...
sequentialRedBody :: Pattern MCMem
                  -> VName
                  -> SegSpace
                  -> [SegBinOp MCMem]
                  -> DoSegBody
                  -> MulticoreGen Imp.Code
sequentialRedBody pat flat_idx space reds kbody = collect $ do
  let (is, ns) = unzip $ unSegSpace space
  ns' <- mapM toExp ns
  zipWithM_ dPrimV_ is $ unflattenIndex ns' $ Imp.vi32 flat_idx
  dScope Nothing $ scopeOfLParams $ concatMap (lambdaParams . segBinOpLambda) reds
