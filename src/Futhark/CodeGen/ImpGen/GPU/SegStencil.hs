...
module Futhark.CodeGen.ImpGen.GPU.SegStencil (compileSegStencil) where

import Control.Monad.Except
import Data.List (group, nub, sort, sortOn, transpose)
...
sForUnflatten sizes start_flat added_flat body = do
  ...
  iterations <- dPrimVE "iterations" $ flat_size `divUp` added_flat - 1
...
compileGlobalReadFlat pat lvl space op kbody = do
  ...
  sKernelThread "segstencil-globalRead" grid_flat group_size_flat_c (segFlat space) $ do
    ...
    sWhen (foldl1 (.&&.) $ zipWith (.<=.) gids max_ixs) $ do
      ...
      ixs_lookup <-
        forM
          (zip3 unique_ixs gids max_ixs)
          ( \(ixs_tup, gid, maxix) ->
              forM
                ixs_tup
                ( \relix ->
                    let read_ix = gid + fromInteger relix
                     in let bounded
        ...
        sKernelThread "multiWriteBigTile" num_groups (Count blocksize) (segFlat space) $ do
          ...
          group_sizes <-
            mapM
              ...
              group_sizes_exp
          group_spans <-
            mapM (dPrimVE "group_spans") $
              createSpans group_sizes
          group_size_flat <-
            dPrimVE "group_size_flat" $
              product group_sizes

          work_sizes <-
            mapM (dPrimVE "work_sizes") $
              zipWith (*) work_multiples_exp group_sizes

          shared_sizes <-
            mapM (dPrimVE "shared_sizes") $
