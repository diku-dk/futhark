...
launchKernel kernel_name kernel_dims workgroup_dims args = do
  ...
  where processKernelArg :: String
        ...
        processKernelArg kernel argnum (Imp.MemKArg v) = do
          err <- newVName' "setarg_err"
          dest <- newVName "k_arg_dest"
          let err_var = Var err
          return [ Comment "MemKArg" []
                 , Fixed (CS.assignScalarPointer (memblockFromMem v) (Var $ CS.compileName dest))
...
copyOpenCLMemory destmem destidx (Imp.Space "device") srcmem srcidx Imp.DefaultSpace nbytes _ = do
  ...
  CS.stm $ Fixed (CS.assignArrayPointer (Var srcmem') (Var ptr))
    [ ifNotZeroSize nbytes $
      Exp $ CS.simpleCall "CL10.EnqueueWriteBuffer"
        [ Var "queue", memblockFromMem destmem, Bool True
...
staticOpenCLArray name "device" t vs = do
  ...
  CS.stm $ Unsafe [
    Fixed (CS.assignArrayPointer (Var tmp_arr) (Var ptr))
      [ ifNotZeroSize size $
        Exp $ CS.simpleCall "CL10.EnqueueWriteBuffer"
          [ Var "ctx.opencl.queue", memblockFromMem static_mem, Bool True
...
futharkSyncContext = Exp $ CS.simpleCall "futhark_context_sync" []
