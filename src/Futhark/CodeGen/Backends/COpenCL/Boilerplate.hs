...
commonOptions =
  [ Option
      { optionLongName = "device",
        ...
        optionDescription = "Use the first OpenCL device whose name contains the given string.\nThe special string #k, where k is an integer, can be used to pick the k-th device, numbered from zero.\nIf used in conjunction with -p, only the devices from matching platforms are considered.",
    ...
    Option
      { optionLongName = "default-group-size",
        ...
        optionDescription = "The default size of OpenCL workgroups that are launched. Capped to the hardware limit if necessary.",
    ...
    Option
      { optionLongName = "default-threshold",
        optionShortName = Nothing,
        optionArgument = RequiredArgument "INT",
        optionDescription = "The default parallelism threshold used for comparisons when selecting between code versions generated by incremental flattening.\nIntuitively, the amount of parallelism needed to saturate the GPU.",
    ...
    Option
      { optionLongName = "size",
        ...
        optionDescription = "Set a configurable run-time parameter to the given value.\nASSIGNMENT must be of the form \"NAME=INT\".\nUse --print-sizes to see which names are available.",
        optionAction =
          [C.cstm|{
                char *name = optarg;
