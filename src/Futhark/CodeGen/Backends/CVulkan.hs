...
compileProg prog = do
  ...
  (ws,)
    <$> GC.compileProg
      "vulkan"
      operations
      (generateBoilerplate prog')
      "#include <vulkan/vulkan.h>"
      [Space "device", DefaultSpace]
      cliOptions
      (hostDefinitions prog')
  where
    operations :: GC.Operations Vulkan ()
    operations =
...
cliOptions =
  commonOptions
    ++ [ Option
           { optionLongName = "dump-spirv",
             optionShortName = Nothing,
             optionArgument = RequiredArgument "FILE",
             optionDescription = "Dump SPIR-V code to this file.",
             optionAction = [C.cstm|futhark_context_config_dump_program_to(cfg, optarg);|]
           },
         Option
           { optionLongName = "lunarg-debug",
             optionShortName = Nothing,
             optionArgument = NoArgument,
             optionDescription = "Enable LunarG debugging.",
...
writeVulkanScalar mem i t "device" _ val = do
  ...
  GC.stm
    [C.cstm|VULKAN_SUCCEED(vkMapMemory(ctx->vulkan.device,
                                            $exp:mem.memory,
                                            $exp:i,
                                            sizeof($ty:t),
                                            0,
...
vulkanMemoryType space =
  error $ "Vulkan backend does not support '" ++ space ++ "' memory space."

staticVulkanArray :: GC.StaticArray Vulkan ()
staticVulkanArray name "device" t (ArrayValues vs) = do
  ...
  -- RESTORE THIS SOMEHOW
  GC.item [C.citem|struct memblock_device $id:name = ctx->$id:name;|]
staticVulkanArray _ space _ _ =
  error $ "Vulkan backend cannot create static array in memory space '" ++ space ++ "'"
...
callKernel (LaunchEntryPoint name args spec_consts (wg_x, wg_y, wg_z)) = do
  ...
  GC.stm
    [C.cstm|VULKAN_SUCCEED(vulkan_available_command_buffer(&ctx->vulkan,
                                                                &$id:cmd_buffer_id));
         |]
  ...
  GC.decl
    [C.cdecl|struct vk_buffer_mem_pair *$id:scalar_buffers =
         ...
         |]
  GC.stm
    [C.cstm|VULKAN_SUCCEED(vulkan_allocate_descriptor_set(&ctx->vulkan,
                                                               &ctx->$id:shader_ctx,
                                                               $id:cmd_buffer_id));|]
  ...
  GC.decl
    [C.cdecl|struct $id:spec_data_struct { $sdecls:spec_const_s_mems }
  ...
  GC.stm
    [C.cstm|
         ...
         |]
  GC.decl
    [C.cdecl|const typename VkWriteDescriptorSet $id:write_desc_set[]
                      = {$inits:wd_inits};
          |]
  GC.stm
    [C.cstm|vkUpdateDescriptorSets(ctx->vulkan.device,
                                        $int:buffer_count,
                                        $id:write_desc_set,
                                        0,
                                        0);
  ...
  GC.stm
    [C.cstm|vulkan_dispatch(&ctx->vulkan,
                                 &ctx->$id:shader_ctx,
                                 $id:cmd_buffer_id,
                                 $id:pipeline,
                                 $exp:wg_x',
...
specMapEntry s d ns = zipWith mapEntry [0 .. length ns - 1] ns
  where
    mapEntry i n = [C.cinit|{$int:i, offsetof($esc:("struct " ++ pretty s), $id:n), sizeof($id:d.$id:n)}|]
...
memArgTransferOwnership cmd_buffer_id (MemKArg bname) =
  GC.stm
