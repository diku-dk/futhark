...
compileOp (Atomic op) = doIspcAtomic op
...
-- Another note: this is not needed. The ispc documentation does not state which atomic_compare_exchange overloads exists
...
-- Note to self: There are atomic variants that take either:
-- all uniform parameters,
-- only uniform type * uniform ptr and varying val,
-- uniform type * varying ptr and varying val. This should cover all the simple atomic operations
doIspcAtomic :: AtomicOp -> ISPCCompilerM ()
doIspcAtomic (AtomicAdd ty old arr idx val) = 
  doAtomic old arr idx val "atomic_add_global" [C.cty|$ty:(GC.intTypeToCType ty)|]
doIspcAtomic (AtomicSub t old arr ind val) =
  doAtomic old arr ind val "atomic_subtract_global" [C.cty|$ty:(GC.intTypeToCType t)|]
doIspcAtomic (AtomicAnd t old arr ind val) =
  doAtomic old arr ind val "atomic_and_global" [C.cty|$ty:(GC.intTypeToCType t)|]
doIspcAtomic (AtomicOr t old arr ind val) =
  doAtomic old arr ind val "atomic_or_global" [C.cty|$ty:(GC.intTypeToCType t)|]
doIspcAtomic (AtomicXor t old arr ind val) =
  doAtomic old arr ind val "atomic_xor_global" [C.cty|$ty:(GC.intTypeToCType t)|]  
doIspcAtomic (AtomicXchg ty old arr ind val) = do 
  ind' <- GC.compileExp $ untyped $ unCount ind
  val' <- GC.compileExp val
...
doIspcAtomic atomic_op@(AtomicCmpXchg t old arr ind res val) = do  
