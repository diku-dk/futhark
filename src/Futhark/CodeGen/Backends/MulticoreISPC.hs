...
compileCode vari code@(Write dest (Count idx) elemtype DefaultSpace vol elemexp)
  ...
  where
    ...
    isSimple _ = False
compileCode _ (Read x src (Count iexp) restype DefaultSpace vol) = do
  src' <- GC.rawMem src
...
compileCode _ code@(Copy dest pt (Count destoffset) DefaultSpace src (Count srcoffset) DefaultSpace (Count size)) = do
