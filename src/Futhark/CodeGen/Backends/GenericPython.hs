...
opaqueDefs (Imp.Functions funs) =
  ...
  where
    ...
    vd (Imp.ScalarValue pt s _) =
      readTypeEnum pt s
    vd (Imp.ArrayValue _ _ _ pt s dims) =
...
entryPointOutput (Imp.TransparentValue (Imp.ArrayValue mem offset (Imp.Space sid) bt ept dims)) = do
  pack_output <- asks envEntryOutput
  offset' <- compileDim offset
  pack_output mem offset' sid bt ept dims
entryPointOutput (Imp.TransparentValue (Imp.ArrayValue mem offset _ bt ept dims)) = do
  mem' <- compileVar mem
  offset' <- compileDim offset
  ...
  pure $
    simpleCall
      ...
      [ Index
          ...
          (IdxRange offset' (BinOp "+" offset' (foldl1 (BinOp "*") dims'))),
        Tuple dims'
      ]
...
declEntryPointInputSizes = mapM_ onSize . concatMap sizes
  where
    ...
    valueSizes (Imp.ArrayValue _ _ _ _ _ dims) = subExpVars dims
...
entryPointInput (i, Imp.TransparentValue (Imp.ArrayValue mem offset (Imp.Space sid) bt ept dims), e) = do
  unpack_input <- asks envEntryInput
  mem' <- compileVar mem
  offset' <- compileDim offset
  unpack <- collect $ unpack_input mem' offset' sid bt ept dims e
  stm $
    Try
      unpack
      [ Catch
...
entryPointInput (i, Imp.TransparentValue (Imp.ArrayValue mem offset _ t s dims), e) = do
...
printValue = fmap concat . mapM (uncurry printValue')
  where
    ...
    printValue' (Imp.TransparentValue (Imp.ArrayValue mem offset (Space _) bt ept shape)) e =
      printValue' (Imp.TransparentValue (Imp.ArrayValue mem offset DefaultSpace bt ept shape)) $
...
entryTypes (Imp.EntryPoint _ res args) =
  ...
  where
    ...
    desc (u, Imp.TransparentValue (Imp.ArrayValue _ _ _ pt s dims)) =
