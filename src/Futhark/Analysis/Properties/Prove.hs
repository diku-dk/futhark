module Futhark.Analysis.Properties.Prove
  ( Property (..),
    prove,
    sumOverIndexFn,
  )
...
import Futhark.Analysis.Properties.AlgebraBridge (addRelIterator, algebraContext, answerFromBool)
import Futhark.Analysis.Properties.IndexFn (Domain (..), IndexFn (..), Iterator (..), cases, flattenCases, guards)
import Futhark.Analysis.Properties.IndexFnPlus (domainEnd, domainStart, intervalEnd, intervalStart, repDomain)
import Futhark.Analysis.Properties.Monad (IndexFnM, printM, printTrace, rollbackAlgEnv)
import Futhark.Analysis.Properties.Query
import Futhark.Analysis.Properties.Rewrite (rewrite)
import Futhark.Analysis.Properties.Substitute qualified as Subst
