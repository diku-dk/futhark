...
simplifyRule3 (View it (Cases cases))
  | Just sops <- mapM (SoP.justConstant . snd) cases =
...
rewriteRule4 (View it@(Forall i'' (Iota _)) (Cases cases))
  | (Var i :== b, e) :| [(Not (Var i' :== b'), x)] <- cases,
    ...
    i == i',
    i == i'',
    b == SoP.int2SoP 0, -- Domain is iota so b must be 0.
    b == b' = do
  ...
  where
    justTermPlusRecurence :: SoP.SoP Exp -> Maybe (SoP.SoP Exp)
    justTermPlusRecurence sop
      | [([x], 1), ([Recurrence], 1)] <- getSoP sop =
          Just (SoP.term2SoP [x] 1)
