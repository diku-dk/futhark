...
analyzeStm m (Let (Pattern [] [PatElem vname _]) _ (Op (Alloc _ _))) =
...
analyzeStm m (Let pat _ (If _ then_body else_body _)) =
  let m' =
   ...
   in zip (patternContextNames pat) (bodyResult then_body)
        <> zip (patternContextNames pat) (bodyResult else_body)
        & mapMaybe (filterFun m')
        & foldr (uncurry addAlias) m'
  where
analyzeStm m (Let pat _ (DoLoop ctx_params _ _ body)) =
  let m_init =
        unzip ctx_params
          & snd
          & zip (patternContextNames pat)
          & mapMaybe (filterFun m)
          & foldr (uncurry addAlias) m
      m_params =
        ctx_params
          & map (first paramName)
          & mapMaybe (filterFun m_init)
          & foldr (uncurry addAlias) m_init
      m_body = analyzeStms (bodyStms body) m_params
      m_res =
        zip (patternContextNames pat) (bodyResult body)
