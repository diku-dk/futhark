...
diffMinMaxHist _ops x aux n minmax ne is vs w rf dst m = do
  ...
  dst_minus_ones <-
    letExp "minus_ones" $
      BasicOp $
        Replicate (Shape dst_dims) $
          intConst Int64 (-1)
  ne_minus_ones <-
    letSubExp "minus_ones" $
      BasicOp $
        Replicate (Shape inner_dims) $
          intConst Int64 (-1)
  iota_n <-
    letExp "red_iota" $
      BasicOp $
...
diffHist ops xs aux n lam0 ne as w rf dst m = do
  ...
  scansres <-
    letTupExp "adj_ctrb_scan" $
      Op $
        Screma n [iota_n] $
          ScremaForm (map (`Scan` ne') scan_lams) [] g_lam
