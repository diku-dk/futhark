...
module Futhark.Backends.GenericC
  ( compileProg
  , ExpCompiler
  , ExpCompilerResult(..)
  , CompilerM
...
import Futhark.Backends.GenericCReading
...
|]
  where compileProg' = do
          ...
          main <- case funDecByName (nameFromString "main") prog of
                    Nothing -> error "Futhark.CCodeGen.compileProg: No main function"
...
compileExp' place (DoLoop respat merge loopvar boundexp loopbody loc) = do
  ...
  (loopbody', respat') <-
    binding ((identName loopvar, varExp loopvar') : bindings) $ do
      loopbody' <- compileBody (varExp mergevarW) loopbody
      respat'   <- compileExp' place $ SubExps (map Var respat) loc
      return (loopbody', respat')
  return $ stm [C.cstm|{
