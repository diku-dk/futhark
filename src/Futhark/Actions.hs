...
-- | The @futhark vulkan@ action.
compileVulkanAction :: FutharkConfig -> CompilerMode -> FilePath -> Action KernelsMem
compileVulkanAction fcfg mode outpath =
  Action
    { actionName = "Compile to Vulkan",
      actionDescription = "Compile to Vulkan",
      actionProcedure = helper
    }
  where
    helper prog = do
      cprog <- handleWarnings fcfg $ CVulkan.compileProg prog
