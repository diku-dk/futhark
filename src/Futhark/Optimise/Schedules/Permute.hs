...
module Futhark.Optimise.Schedules.Permute
  ( applyPermute
  )
where
...
import Futhark.Optimise.Schedules.EnvUtils
  (TopEnv(..), invPerm, identityPerm)
import Futhark.Optimise.Schedules.SchedUtils
  ( HLSched(..), oneFullyConsumedMapRed, append2Sched,
    headOfSched, tailOfSched, sortByPerm, equivLambdas,
    stmtMatchesSchedule, fromFParam )

import Debug.Trace

applyPermute :: 
    (LocalScope SOACS m, MonadFreshNames m) =>
    TopEnv -> HLSched -> StmAux (ExpDec SOACS) -> Exp SOACS -> m (Stms SOACS)
applyPermute env sched aux e_soac
  | Op soac <- e_soac,
  ...
  where
    ...
    --
    findSOACtype (Screma mm _inp_nms (ScremaForm map_lam [] [])) =
      map (`arrayOfRow` mm) $ lambdaReturnType map_lam
    findSOACtype (Screma _mm _inp_nms form)
...
--
applyPermute _ _ _ e_soac =
  error ("Unsupported argument to applyPermute:\n"++prettyString e_soac)
...
permuteNest env sched orig_nest_stm@(Let _pat _aux e_soac)
  ...
  --   down to permuting the last recurrence of the current soac. 
    Just (code_bef, rec_stm, code_after, res_ses) <- trace "\nPERMUTE CASE 3\n" $ sep3LastRec env sched e_soac = do
  ...
  --        permutation s0 ++ sort(s1:_rest) to the whole nest
    ...
    last_rec_stms <- trace ("Permute Case 1.0, s0 < s1") $
      ...
      permuteNest env sched_rec rec_stm
  ...
  -- ^ interchange the front dimensions, then call recursively on the top soac
    ...
    (before_stms, last_rec_stm, after_stms) <- trace ("Permute Case 2.0, s0 > s1") $
      ...
      transposeTopOfNest env sched orig_nest_stm
    last_rec_stms' <- trace ("Permute Case 2.0, s0 > s1, recurse on sched") $ 
      ...
      permuteNest env (interchangeTop sched) last_rec_stm
...
transposeTopOfNest :: (LocalScope SOACS m, MonadFreshNames m) =>
    TopEnv -> HLSched -> Stm SOACS -> m (Stms SOACS, Stm SOACS, Stms SOACS)
transposeTopOfNest env sched outer_rec_stm
  ...
  --
  let tab2 = tab1 `M.union` tab_schd
...
transposeTopOfNest _env _sched outer_rec_stm =
  error ("Yet unsuported case for function transposeTopOfNest in Permute.hs, stmt:\n"++prettyString outer_rec_stm++"\n\n")
...
distributeMapOnStmts _ _ (inner_stms, _) res_nms
  | null inner_stms || res_nms == mempty = pure (mempty, mempty)
...
interchangeInwards tab out_rec_stm (inn_rec_stm, inline_stms) res_nms
  ...
  (tab', out_pat_nms') <-
      ...
      -- the result of the outer map lambda is exactly the result of the inner redomap
...
sep3LastRec :: TopEnv -> HLSched -> Exp SOACS -> Maybe (Stms SOACS, Stm SOACS, Stms SOACS, [SubExp])
...
sep5LastRecBody :: TopEnv -> HLSched -> Body SOACS -> (PartStms, [SubExp])
...
-- | This is supposed to make a body from `stms_bef`, `stms_perm_rec` and
--     `stms_aft`, where `stms_perm_rec` are the statements generated by
...
findCheapStms2Inline fvs_body parts
  ...
  where
    ...
    findCheapStm (cheap_nms, cheap_stms) stm@(Let pat _ e) =
      let (ok_stm2inline, used_nms) = namesInInlineStm pat e
    ...
    --
    namesInInlineStm _ (BasicOp (SubExp (Constant _))) = (True, [])
    namesInInlineStm _ (BasicOp (SubExp (Var nm))) = (True, [nm])
    namesInInlineStm _ (BasicOp (UnOp _ se))       = (True, mapMaybe se2VName [se])
    namesInInlineStm _ (BasicOp (ConvOp _ se))     = (True, mapMaybe se2VName [se])
    namesInInlineStm _ (BasicOp (BinOp _ se1 se2)) = (True, mapMaybe se2VName [se1,se2])
    namesInInlineStm _ (BasicOp (CmpOp _ se1 se2)) = (True, mapMaybe se2VName [se1,se2])
    namesInInlineStm _ (BasicOp (Assert se _ _))   = (True, mapMaybe se2VName [se])
    namesInInlineStm (Pat [patel]) (BasicOp (Index arrnm slice)) =
      (not (primType $ patElemDec patel), arrnm : concat (map namesFromDim (unSlice slice)))
      where
        namesFromDim (DimFix se) = mapMaybe se2VName [se]
        namesFromDim (DimSlice se1 se2 se3) = mapMaybe se2VName [se1,se2,se3]
    namesInInlineStm _ _ = (False, [])
    --
