...
lowerUpdate (Let pat _ (LoopOp (DoLoop res merge i bound body))) updates = do
  ...
  Just $ do
    ...
    return [mkLet pat' $ LoopOp $ DoLoop res' merge' i bound body']
lowerUpdate
...
lowerUpdateIntoLoop updates pat res merge body = do
  ...
  Just $ do
    ...
    (bodyresult, new_body_bnds) <- manipulateResult in_place_map mergeparams
