...
lastUseAnBdy alstab bdy@(Body _ bnds result) (lutab, used_nms) =
  ...
  where
    ...
    traverseBindings stab Seq.Empty (lutab1, nms) res_nms =
      let nms' =
            foldlNames
              ( \acc x -> case M.lookup x stab of
                  Nothing -> acc
                  Just al -> acc <> al
              )
              nms
  ...
  let free_in_body = aliasTransClos alstab $ freeIn body
      ...
      var_inis_a =
        filter
          ( \(_, nms) ->
              (== mempty) $
                namesIntersection nms $
                  free_in_body <> used_nms0
          )
          var_inis
      ...
      lutab_res = body_lutab <> M.fromList var_inis_a

      -- the result used names are:
      fpar_nms = namesFromList $ map (identName . paramIdent) $ map fst var_ses
...
{-
myFun :: AliasTab -> Exp (Aliases Mem.InKernel)
             -> Names -> (LUTabFun,Names,Names)
myFun alstab (Op (Mem.Inner (Mem.SplitArray _ _ _ _ _ _))) used_nms = (M.empty, S.empty, used_nms)
-}
        ...
        foldlNames
          ( \acc x ->
              let x_alias = case M.lookup x alstab of
                    Nothing -> oneName x
                    Just al -> oneName x <> al
                  is_unused = (== mempty) $ namesIntersection used_nms x_alias
               in if is_unused then acc <> x_alias else acc
          )
          mempty
          args
