  ...
  where
    ...
    onBdy scope0 lu_tab body = do
      let td_env = TopDownEnv  mempty []
          bu_env = BottomUpEnv mempty mempty mempty
      (_, stms) <- 
        modifyNameSource $
          runState $
...
fuseIScatStms (td_env, bu_env) (stm :<| stms) = do
  ...
  (env'', stm') <- fuseIScatStm env' stm
  pure (env'', oneStm stm' <>  stms')

fuseIScatStm :: (TopDownEnv, BottomUpEnv GPU) -> Stm GPU -> FuseIScatM ( (TopDownEnv, BottomUpEnv GPU), Stm GPU )
fuseIScatStm (td_env, bu_env) stm = do
  pure $ ((td_env, bu_env), stm)

fuseIScatBdy :: (TopDownEnv, BottomUpEnv GPU) -> Body GPU -> FuseIScatM ( (TopDownEnv, BottomUpEnv GPU), Body GPU )
fuseIScatBdy env (Body () stms res) = do
  (env, stms') <- fuseIScatStms env stms
  return $ (env, Body () stms' res)
