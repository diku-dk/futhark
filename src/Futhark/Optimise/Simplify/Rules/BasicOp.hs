...
ruleBasicOp vtable pat aux (Update safety src (DimIndices [DimSlice i n s]) (Var v))
  | isCt1 n,
    isCt1 s,
    ...
    Simplify $ do
      ...
      auxing aux $
        certifying cs $
          letBind pat $ BasicOp $ Update safety src (DimIndices [DimFix i]) e'
...
ruleBasicOp vtable pat aux (Update safety arr_x (DimIndices slice_x) (Var v))
  | Just _ <- sliceIndices $ DimIndices slice_x,
    ...
    certifying cs_y $
      auxing aux $
        letBind pat $ BasicOp $ Update safety arr_x (DimIndices slice_x') $ Var v'
