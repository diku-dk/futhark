...
genRed2MapRed env kerstm@(Let pat_ker aux (Op (SegOp (SegMap seg_thd seg_space kres_tps old_kbody))))
  | (SegThread _ seg_group_size _novirt) <- seg_thd,
    ...
    --   is redundant than its a no go. Otherwise we need to look at
    ...
    (k1_res, ker1_stms) <- runBuilderT' $ do
        ...
        res_redmap <- letTupExp "res_mapred" $ Op (OtherOp (Screma inv_dim_len [iota] (ScremaForm [] [red] map_lam)))
    ...
    let gid_dims_1 = 
          filter (\x -> invar_gid /= (fst x)) $
          unSegSpace seg_space
    let space1 = SegSpace gid_flat_1 gid_dims_1

    (grid_size, host_stms1) <- runBuilder $ do
        let grid_pexp = foldl (\x d -> x * pe64 d) (pe64 se1) $ map snd gid_dims_1
