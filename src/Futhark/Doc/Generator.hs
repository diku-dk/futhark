...
indexPage pages = H.docTypeHtml $ addBoilerplate "/" "Futhark Library Documentation" $
                  H.ul (mconcat $ map linkTo $ sortBy (comparing fst) pages)
  where linkTo (name, _) =
          let file = makeRelative "/" $ name -<.> "html"
          in H.li $ H.a ! A.href (fromString file) $ fromString name
