...
newtype InnerSimplifyM a
  = InnerSimplifyM (StateT (M.Map Size VName) SimplifyM a)
  deriving
    ( Monad,
      Functor,
      Applicative,
...
runInnerSimplifier :: InnerSimplifyM a -> M.Map Size VName -> SimplifyM (a, M.Map Size VName)
runInnerSimplifier (InnerSimplifyM m) = runStateT m

instance MonadFreshNames InnerSimplifyM where
  getNameSource = InnerSimplifyM $ StateT (((,) <$> getNameSource <*>) . pure)
  putNameSource src = InnerSimplifyM $ StateT (((,) <$> putNameSource src <*>) . pure)
...
hardOnRetType :: S.Set VName -> RetTypeBase Size as -> InnerSimplifyM (RetTypeBase Size as)
...
expFree :: S.Set VName -> Exp -> InnerSimplifyM Exp
...
onResType :: S.Set VName -> AppRes -> InnerSimplifyM AppRes
onResType scope (AppRes ty ext) =
  AppRes <$> onType (scope `S.union` S.fromList ext) ty <*> pure ext

onPat :: S.Set VName -> Pat -> InnerSimplifyM Pat
onPat scope =
  astMap mapper
  where
    mapper =
...
onRetType :: S.Set VName -> S.Set VName -> RetTypeBase Size as -> InnerSimplifyM (RetTypeBase Size as)
...
onScalar :: S.Set VName -> ScalarTypeBase Size as -> InnerSimplifyM (ScalarTypeBase Size as)
...
onType scope (Array as u shape scalar) =
  Array as u <$> mapM onSize shape <*> onScalar scope scalar
...
removeExpFromValBind valbind = do
  ...
  (params', expNaming) <- runInnerSimplifier (mapM (onPat scope) $ valBindParams valbind) mempty
  let typeParams' =
        valBindTypeParams valbind
          <> map (\(e, vn) -> TypeParamDim vn (srclocOf $ unSizeExpr e)) (M.toList expNaming)
  let args = foldMap patArg params'
  (rety', _) <- runInnerSimplifier (hardOnRetType (scope `S.union` args) $ unInfo $ valBindRetType valbind) expNaming
  let scope' = scope `S.union` args
  (body', expNaming') <- runInnerSimplifier (expFree scope' $ valBindBody valbind) expNaming
