...
intrinsics =
          ...
          ++ [ ( "flatten",
               ...
               ( "stencil_3d",
                 IntrinsicPolyFun
                   ...
                   $ RetType [] $ shape [n, m, z] `uarray` t_b
               ),
               ( "map_stream",
                 IntrinsicPolyFun
                   [tp_a, tp_b, sp_n]
  ...
  where
    [a, b, c, n, m, z, k, l, p, q] =
      zipWith VName (map nameFromString ["a", "b", "c", "n", "m", "z", "k", "l", "p", "q"]) [0 ..]
    ...
    [sp_n, sp_m, sp_z, sp_k, sp_l, sp_p, sp_q] = map (`TypeParamDim` mempty) [n, m, z, k, l, p, q]
